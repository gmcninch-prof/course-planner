...
def entryToOrg (courseName : String) (day : AcademicDay) (entry : CalEntry) : String :=
  match entry with
...
def dayToOrg (courseName : String) (day : AcademicDay) : String :=
  day.entries.map (entryToOrg courseName day) |> "\n".intercalate
...
def courseCalendarToOrg (cc : CourseCalendar) : String :=
  ...
  let body := cc.days.map (dayToOrg courseName) |> "\n".intercalate
  header ++ "\n" ++ body
